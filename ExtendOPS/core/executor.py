...
Core execution logic for Extended Exchange
"""
...
from x10.perpetual.configuration import MAINNET_CONFIG, TESTNET_CONFIG, MAINNET_CONFIG_LEGACY_SIGNING_DOMAIN
...
class ExtendedExecutor:
    ...
    async def initialize(self):
        ...
        try:
            # Load configuration
            config_path = (
                "/home/kt/ZKputer/ExtendOPS/config/trading_config.json"
            )
            ...
            elif self.config.get("legacy_signing", False):
                exchange_config = MAINNET_CONFIG_LEGACY_SIGNING_DOMAIN
            else:
                exchange_config = MAINNET_CONFIG
    ...
    async def set_leverage(self, symbol, leverage):
        ...
        try:
            # Check current leverage first to avoid redundant calls
    ...
    async def place_order(self, symbol, side, size, price, slippage=0.05):
        ...
        try:
            # Place limit order
            # If side is string, convert to OrderSide
            ...
            try:
                ...
                for pos in positions.data:
                    if pos.market == symbol and float(pos.size) > 0:
                        print(f"⚠️ Position already exists for {symbol}: {pos.size}")
                        return {
                            "status": "ok",
                            "order_id": "existing",
                            ...
                            "symbol": symbol,
                            "side": side,
                            "note": "Position already existed"
                        }
            except Exception as e:
                print(f"⚠️ Failed to check existing positions: {e}")

            # Calculate expiration (mandatory)
            # Mainnet: 90 days, Testnet: 28 days
            is_testnet = self.config.get("testnet", False)
            days = 28 if is_testnet else 90
            expiration_timestamp = int((datetime.now(timezone.utc) + timedelta(days=days)).timestamp() * 1000)

            # Handle Market Orders (Simulated as Limit IOC)
            time_in_force = TimeInForce.GTT
            final_price = Decimal(str(price))
            
            # If price is 0 or None, treat as Market Order
            if price is None or price == 0:
                # We need a reference price. For now, we rely on the user passing a price even for market orders
                # as per our new protocol. If they don't, we can't safely execute without fetching orderbook.
                # Assuming the passed 'price' IS the intended execution price (with buffer if market)
                pass

            # If it's a "market" order execution, we use IOC and a buffered price
            # But here we receive a specific price.
            # Let's assume the caller (execute_trades.py) has already calculated the buffer price for market orders.
            # We just need to set IOC if it's intended to be a market fill.
            # For now, we'll default to GTT unless specified.
            
            # Fee (Mandatory) - Taker fee assumption 0.001 (0.1%) to be safe
            fee = Decimal("0.1") 
            ...
            print(f"✅ Order placed! ID: {placed_order.data.id}")
    ...
    async def update_performance_log(self, results, timestamp=None):
        ...
        log_path = "/home/kt/ZKputer/ExtendOPS/docs/knowledge_graph/performance_log.md"
